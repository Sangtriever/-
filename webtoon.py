...
def homework_post():
    ...
    db.suzy.insert_one(doc)
    return jsonify({'msg': '입력 완료!'})


@app.route("/homework", methods=["GET"])
def homework_get():
    suzy_list = list(db.suzy.find({}, {'_id': False}))
    return jsonify({'suzy': suzy_list})
...
@app.route("/boardcheak", methods=["GET"])
def board():
    all_board = list(db.board.find({}, {'_id': False}))
    return jsonify({'all_boards': all_board})
